...
export function createCampaignRouter(
  ...
  const campaignRouter = Router({ mergeParams: true });
  ...
  campaignRouter.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      const snapshots = await db.campaignRecipientSnapshot.findMany({
        where: {
          campaignId,
          businessId,
          ownerId: req.user!.id,
  ...
  campaignRouter.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      if (hasSnapshot) {
        ...
        attemptedSendCount =
          sentLogs.length + failedDeliveryCount + pendingLogs.length;
        ...
        const skippedLogs = logs.filter(
          (log) =>
            log.status === "FAILED" &&
            isSkippedBeforeSendReason(log.errorMessage),
        );
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      if (result.count === 0) {
        return res.status(404).json({
          error: "Campaign not found. Queued or sent campaigns cannot be edited.",
        });
      }
  ...
  campaignRouter.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      await db.campaignRecipientSnapshot.deleteMany({
        where: {
          campaignId,
          businessId,
          ownerId: req.user!.id,
      ...
      await db.campaignRecipientSnapshot.createMany({
        data: snapshots,
      });
  ...
  campaignRouter.delete(
    ...
    async (req: AuthenticatedRequest, res) => {
      ...
      if (result.count === 0) {
        return res.status(404).json({
          error: "Campaign not found. Queued or sent campaigns cannot be deleted.",
        });
      }
...
export const campaignRouter = createCampaignRouter();
